...
- **The simulated aircraft (MOCK911) only starts moving once a target LZ is searched**, and only re-computes its course every ~3s (the simulation tick). Immediately after selecting a search suggestion, screenshot again after `Start-Sleep -Seconds 3-8` before concluding anything about its position/label — a screenshot taken too soon just shows the pre-search parked state, which looks identical to a stuck/broken state at a glance.
